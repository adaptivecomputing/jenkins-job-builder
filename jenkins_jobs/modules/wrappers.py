...
def mongo_db(parser, xml_parent, data):
    ...
    :arg str port: Port for the server (optional)
    :arg str startup-params: Startup parameters for the server (optional)
    :arg str start-timeout: How long to wait for the server to start in 
        milliseconds. 0 means no timeout. (default '0')

    Example:

    .. literalinclude:: /../../tests/wrappers/fixtures/mongo_db001.yaml

    """
    mongodb = XML.SubElement(xml_parent,
                              'org.jenkinsci.plugins.mongodb.'
                              'MongoBuildWrapper')
    mongodb.set('plugin', 'mongodb@1.3')

    XML.SubElement(mongodb, 'mongodbName').text = str(data.get('name', ''))
    XML.SubElement(mongodb, 'dbpath').text = str(data.get('data-directory', ''))
    XML.SubElement(mongodb, 'port').text = str(data.get('port', ''))
    XML.SubElement(mongodb, 'parameters').text = str(data.get('startup-params', ''))
    XML.SubElement(mongodb, 'startTimeout').text = str(data.get('start-timouet', '0'))
