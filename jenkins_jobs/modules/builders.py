...
def doxygen(parser, xml_parent, data):
    ...
    XML.SubElement(doxygen, 'continueOnBuildFailure').text = \
      str(data.get('ignorefailure', False)).lower()
    XML.SubElement(doxygen, 'unstableIfWarnings').text = \
      str(data.get('unstablewarning', False)).lower()
...
def conditional_step(parser, xml_parent, data):
    ...
    def build_condition(cdata):
        ...
        if kind == "always":
            ...
            XML.SubElement(ctag, "ignoreCase").text = str(cdata.get('condition-case-insensitive', False)).lower()
